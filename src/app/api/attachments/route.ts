...
export async function POST(req: NextRequest) {
    ...
    // Generate signed URL
    const fileName = `${file.name.toLowerCase().replace(/\s/g, '').normalize('NFD').replace(/\p{Mn}/gu, '')}` // Generated name of the file
    ...
    return NextResponse.json({ signedUrl, filePath: `https://pasacademy.storage.googleapis.com/${fileName}`  })
}
